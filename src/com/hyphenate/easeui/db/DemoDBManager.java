...
public class DemoDBManager {
	...
	synchronized public List<InviteMessage> getMessagesList() {
		...
		if (db.isOpen()) {
			Cursor cursor = db.rawQuery("select * from " + InviteMessgeDao.TABLE_NAME + " order by time desc", null);
			while (cursor.moveToNext()) {
				...
				if (status == InviteMesageStatus.GROUPINVITATION.ordinal())
					msg.setStatus(InviteMesageStatus.GROUPINVITATION);
				if (status == InviteMesageStatus.GROUPINVITATION_ACCEPTED.ordinal())
