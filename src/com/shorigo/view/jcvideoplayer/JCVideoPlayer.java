...
public class JCVideoPlayer extends FrameLayout implements View.OnClickListener, SeekBar.OnSeekBarChangeListener, SurfaceHolder.Callback, View.OnTouchListener, JCMediaManager.JCMediaPlayerListener {
	...
	private void init(Context context) {
		View.inflate(context, R.layout.jcvideoplayer_video_control_view, this);
	 ...
	 *            封面 | cover
	 */
	public void setUp(String url, String title, String cover) {
		setUp(url, title, cover, true);
	}
	...
	public void setUp(String url, String title, boolean ifShowTitle) {
		...
		if (ifFullScreen) {
			...
			ivFullScreen.setImageResource(enlargRecId == 0 ? R.drawable.jcvideoplayer_shrink_video : enlargRecId);
		} else {
			ivFullScreen.setImageResource(shrinkRecId == 0 ? R.drawable.jcvideoplayer_enlarge_video : shrinkRecId);
	 ...
	 *            是否在非全屏下显示标题 | The title is displayed in full-screen under
	 */
	public void setUp(String url, String title, String cover, boolean ifShowTitle) {
		this.ifShowTitle = ifShowTitle;
		if ((System.currentTimeMillis() - clickfullscreentime) < FULL_SCREEN_NORMAL_DELAY)
			return;
		this.url = url;
		...
		BitmapHelp.loadImg(context, ivCover, cover);
		tvTitle.setText(title);
	...
	public void setUpForFullscreen(String url, String title) {
		...
		CURRENT_STATE = CURRENT_STATE_NORMAL;
		if (ifFullScreen) {
			ivFullScreen.setImageResource(shrinkRecId == 0 ? R.drawable.jcvideoplayer_shrink_video : shrinkRecId);
		} else {
			ivFullScreen.setImageResource(enlargRecId == 0 ? R.drawable.jcvideoplayer_enlarge_video : enlargRecId);
		}
		tvTitle.setText(title);
		if (!TextUtils.isEmpty(url) && url.contains(".mp3")) {
			ifMp3 = true;
	...
	public void mute() {
		JCMediaManager.intance().mute();
	}

	public void unMute() {
		JCMediaManager.intance().unMute();
	}
	...
	public void onClick(View v) {
		...
		if (i == R.id.start || i == R.id.thumb) {
			...
			if (CURRENT_STATE == CURRENT_STATE_NORMAL || CURRENT_STATE == CURRENT_STATE_ERROR) {
				...
				changeUiToShowUiPrepareing();
				iv_volume.setVisibility(View.INVISIBLE);
	...
	private void updateStartImage() {
		if (CURRENT_STATE == CURRENT_STATE_PLAYING) {
			ivStart.setImageResource(R.drawable.jcvideoplayer_click_video_pause_selector);
		} else if (CURRENT_STATE == CURRENT_STATE_ERROR) {
			ivStart.setImageResource(R.drawable.jcvideoplayer_click_video_error_selector);
		} else {
			ivStart.setImageResource(R.drawable.jcvideoplayer_click_video_play_selector);
		}
	}
