				...
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
							...
							Toast.makeText(CallUI.this, st2, Toast.LENGTH_SHORT).show();
							finish();
						}
					});
				}
			...
			case MSG_CALL_ANSWER:
				...
				if (isInComingCall) {
					try {
						...
						isAnswered = true;
						// meizu MX5 4G, hasDataConnection(context) return
						// status is incorrect
						// MX5
						// con.getNetworkInfo(ConnectivityManager.TYPE_WIFI).isConnected()
						// return false in 4G
						// so we will not judge it, App can decide whether judge
						// the network status
	...
	protected int playMakeCallSounds() {
		try {
			...
			int id = soundPool.play(outgoing, // sound resource
					...
					1); // playback rate (1.0 = normal playback, range 0.5 to
						// 2.0)
			return id;
		} catch (Exception e) {
			return -1;
