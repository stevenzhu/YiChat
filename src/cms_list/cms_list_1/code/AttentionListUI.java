 ...
 * 
 */
public class AttentionListUI extends BaseUI implements OnPullListener, OnItemClickListener {
	...
	protected void prepareData() {
		...
		listCmsBean = gson.fromJson(ACache.get(this).getAsString("attention_list_1_" + type), listCmsTemp);

		adapter = new AttentionListAdapter(this, listCmsBean, mHandler,type);
	...
	private void getCmslist() {
		String url = HttpUtil.getUrl("/user/getConcernInfos");
		...
		HttpUtil.post(this, url, map, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				...
				if (HttpUtil.isSuccess(AttentionListUI.this, returnBean.getCode())) {
					List<CmsBean> listTemp = returnBean.getListObject();
					if (listTemp != null && listTemp.size() == 100) {
					...
					if (isRef) {
						...
						ACache.get(AttentionListUI.this).put("attention_list_1_" + type, json);
					} else {
